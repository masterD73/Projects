...
logging.basicConfig(filename='logs/weatherapp.log',
...
async def insert_db(db, data, location):
    db.weather.insert_one({"Data": data, "location": location})
    logging.info(f"Inserted data for location: {location}")
    return db


async def find_db(db, location):
    db.weather.find_one({"location": location})
    logging.info(f"Found data for location: {location}")
    return db
...
async def weather():
    ...
    logging.info(f"Received request for city: {city}")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(geo_url) as resp:
                print(resp.status)
                logging.info(f"Geo API response status: {resp.status}")
                geo = await resp.text()
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(source_url) as resp:
                print(resp.status)
                logging.info(f"Weather API response status: {resp.status}")
        ...
        await insert_db(mdb, data, location)
        logging.info(f"Rendered template with data for location: {location}")
